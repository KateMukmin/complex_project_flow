...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .bio-body-text { font-size: 13px; color: #2B2A29; line-height: 1.8; margin-bottom: 24px; }
  .bio-check-row { display: flex; align-items: flex-start; gap: 12px; margin-bottom: 24px; }
  .bio-checkbox { width: 18px; height: 18px; min-width: 18px; border-radius: 3px; border: 1.5px solid rgba(43,42,41,0.35); background: #fff; display: flex; align-items: center; justify-content: center; margin-top: 2px; cursor: pointer; transition: all 0.15s; flex-shrink: 0; }
  .bio-checkbox.checked { background: #1BB161; border-color: #1BB161; }
  .bio-checkbox svg { width: 11px; height: 11px; }
  .sig-section { border-top: 1px solid rgba(43,42,41,0.08); padding-top: 20px; margin-bottom: 16px; }
  .sig-section-title { font-size: 13px; font-weight: 600; color: #2B2A29; margin-bottom: 4px; }
  .sig-section-sub { font-size: 11px; color: #727271; margin-bottom: 12px; }
  .apply-sig-btn { font-size: 12px; font-weight: 500; color: #00346B; background: none; border: 1px solid rgba(0,52,107,0.25); border-radius: 100px; padding: 7px 16px; cursor: pointer; font-family: 'Poppins', sans-serif; display: inline-flex; align-items: center; gap: 6px; margin-bottom: 14px; transition: border-color 0.15s; }
  .apply-sig-btn:hover { border-color: #00346B; }
  .apply-sig-btn svg { width: 13px; height: 13px; }
  .sig-display { border: 1px solid rgba(43,42,41,0.15); border-radius: 8px; background: rgba(228,231,233,0.20); padding: 12px 16px; display: none; }
  .sig-display.visible { display: block; }
  .sig-display canvas { display: block; width: 100%; }
  .sig-applied-note { font-size: 10px; color: #1BB161; margin-top: 6px; display: flex; align-items: center; gap: 5px; }
  .sig-applied-note svg { width: 12px; height: 12px; }
</style>
</head>
<body>
        ...
        <div class="proj-card">
          <div class="proj-card-header">
            <div class="proj-card-name">
              ...
              <div class="proj-name-text">Yankee Stadium Project</div>
            </div>
            <div class="menu-btn">
    ...
    <div class="flow-main">
      <div class="flow-content">
        ...
        <div class="proj-info-field">
          ...
          <div class="proj-info-value">Yankee Stadium Project</div>
        </div>
        <div class="proj-info-field">
    ...
    <div class="flow-main">
      <div class="flow-content">
        ...
        <div class="actions">
          ...
          <button class="btn-next" onclick="goTo('page-biometric')">
            Next
            <svg viewBox="0 0 13 13" fill="none"><path d="M2.5 6.5h8M7.5 3.5l3 3-3 3" stroke="#fff" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/></svg>
          </button>
        </div>
...
<!-- ══════════════════════════════════════
     SCREEN 6 — Biometric Consent Form
══════════════════════════════════════ -->
<div class="page" id="page-biometric">
  ...
  <div class="flow-shell">
    <aside class="sidebar" id="sidebar-biometric"></aside>
    <div class="flow-main">
      <div class="flow-content">
        <div class="flow-title">Yankee Stadium Project Biometric Consent Form</div>
        <div class="flow-sub">Enter your consent information</div>

        <div class="bio-body-text">
          My information may be collected, captured, obtained, stored, and used by Yankee Stadium Project ("Yankee Stadium Project") and for identification purposes (collectively, the "Data Users"). Yankee Stadium Project may collect, capture, obtain, store, transmit, maintain, process, derive, and use my biometric information for its use related to the Services or for identification and credentialing. This information includes personal information and biometric information such as digital photograph. Facial recognition, the virtual comparison of your facial images when logging into the Yankee Stadium Project mobile application for allowing access to the online training platform or to tracking your progress through the training and recording your assessment results may be used. My information will be retained by Yankee Stadium Project until I request, in writing, that Yankee Stadium Project delete such information. I may contact a Yankee Stadium Project representative to request removal of my information from Yankee Stadium Project's database. By consenting to the collection and use of this information, I acknowledge and agree that Yankee Stadium Project, and its representatives, may use and share biometric information for the purposes listed above.
        </div>

        <div class="bio-check-row">
          <div class="bio-checkbox" id="bioChk" onclick="toggleBioChk()"></div>
          <div class="check-label">By checking the box below, I give my consent as a written release for Yankee Stadium Project to collect, capture, store, transmit, maintain, process, derive and use my personal biometric information as described above.</div>
        </div>

        <div class="sig-section">
          <div class="sig-section-title">Signature</div>
          <div class="sig-section-sub">Apply the signature you provided during account registration.</div>
          <button class="apply-sig-btn" onclick="applySig()">
            <svg viewBox="0 0 13 13" fill="none"><path d="M2 10L5 7l2 2 4-6" stroke="#00346B" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg>
            Apply Signature
          </button>
          <div class="sig-display" id="sigDisplay">
            <canvas id="sigReplay" height="90"></canvas>
            <div class="sig-applied-note">
              <svg viewBox="0 0 12 12" fill="none"><circle cx="6" cy="6" r="5.5" fill="rgba(27,177,97,0.15)"/><path d="M3.5 6l2 2L8.5 4" stroke="#1BB161" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg>
              Signature applied from your profile
            </div>
          </div>
        </div>
        ...
        <div class="spacer"></div>
        <div class="actions">
          <button class="btn-back" onclick="goTo('page-consent-form')">Back</button>
...
function startFlow() {
  renderSidebar('sidebar-personal',      'Personal Information', 'Yankee Stadium Project');
  renderSidebar('sidebar-contact',       'Contact Information',  'Yankee Stadium Project');
  renderSidebar('sidebar-certs',         'Certifications',       'Yankee Stadium Project');
  renderSidebar('sidebar-consent',       'Consent Form',         'Yankee Stadium Project');
  renderSidebar('sidebar-consent-form',  'Consent Form',         'Yankee Stadium Project');
  renderSidebar('sidebar-biometric',      'Consent Form',         'Yankee Stadium Project');
  goTo('page-personal');
}
...
function toggleBioChk() {
  const el = document.getElementById('bioChk');
  el.classList.toggle('checked');
  el.innerHTML = el.classList.contains('checked')
    ? \`<svg viewBox="0 0 11 11" fill="none"><path d="M2 5.5l2.5 2.5L9 3" stroke="#fff" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"/></svg>\`
    : '';
}

// Signature from the onboarding flow — a simple pre-drawn path replayed on canvas
function applySig() {
  const display = document.getElementById('sigDisplay');
  const canvas = document.getElementById('sigReplay');
  ...
  ctx.lineCap = 'round';
  ctx.lineJoin = 'round';
  // Replay a representative signature path matching what's drawn in the onboarding flow
  const pts = [
    [30,60],[38,40],[48,30],[55,35],[50,50],[45,65],[48,70],[55,60],[65,40],[75,30],[82,35],
    [80,50],[75,62],[80,65],[90,60],[105,45],[118,35],[125,40],[122,55],[118,65],[125,68],
    [140,55],[160,38],[175,32],[182,38],[178,52],[172,65],[178,68],[195,55],[220,38],[240,45]
  ];
  ctx.beginPath();
  ...
  // Change button to show it's been applied
  const btn = document.querySelector('.apply-sig-btn');
  btn.innerHTML = \`<svg viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="5.5" fill="rgba(27,177,97,0.15)"/><path d="M3.5 6.5l2 2L9.5 4" stroke="#1BB161" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round"/></svg> Signature Applied\`;
  btn.style.color = '#1BB161';
  btn.style.borderColor = 'rgba(27,177,97,0.3)';
  btn.onclick = null;
}
...
<div class="modal-overlay" id="welcomeModal">
  <div class="modal">
    ...
    <div class="modal-assigned">You are now assigned to <span>Yankee Stadium Project</span></div>
